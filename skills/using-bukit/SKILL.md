...
**Default behavior (user does NOT explicitly say "use starter theme" or "use the default theme"):**

The Agent must create a brand new custom theme from scratch using bukit-theme's guidance. Do NOT reuse the starter theme's visual design. The site should have a unique, fresh appearance distinct from the default starter look.

When creating a custom theme:
1. Run `bukit init <dir>` to scaffold the project structure (this creates `themes/starter/` for reference only)
2. Create a new theme directory `themes/<custom-name>/` with a unique visual identity
3. Update site.yaml `theme.name` to `<custom-name>`
4. Design new layouts, CSS, and partials from scratch — do not copy-paste starter theme files

**Exception (user explicitly says "use starter theme" / "use the default theme"):**

Only when the user explicitly requests it, use the starter theme as-is generated by `bukit init`.
